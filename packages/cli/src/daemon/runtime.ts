...
export function buildPromptFromHistory(messages: SessionMessage[], nextMessage: string): string {
  ...
  return [
    'Continue the existing local session using the transcript below.',
...
export class DaemonRuntime {
  ...
  async execute(input: ExecuteSessionInput, emit: (event: RuntimeStreamEvent) => void): Promise<ExecuteSessionResult> {
    ...
    return this.enqueueSession(session.id, async () => {
      ...
      try {
        ...
        const prompt = this.buildPrompt(updatedSession, input.message, agent);
  ...
  private buildPrompt(session: SessionRecord, message: string, agent?: DaemonAgent): string {
    let base: string;
    ...
    if (session.claudeResumeId) {
      ...
      base = history.length === 0 ? message : buildPromptFromHistory(history, message);
    }
