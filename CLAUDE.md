...
```
本地 agent 项目 → agent-mesh daemon 持有 agent/session/task 状态
     ↓
owner 通过 agent-mesh chat/call/session/task 管理本地工作
     ↓
需要上线时执行 agent-mesh agent expose <ref> --provider agents-hot
     ↓
