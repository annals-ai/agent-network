...
Agent Mesh is now a daemon-first local runtime for managing many agents and many sessions on one machine, with optional provider exposure such as Agents Hot.
...
The daemon owns local state in SQLite. Local `chat` and `call` go through the daemon first. Exposed providers forward traffic into the same session core instead of bypassing it.
