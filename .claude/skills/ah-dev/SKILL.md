...
description: |
  ...
  daemon runtime, or platform integration in the agent-network sub-repo.
version: 0.0.6
---

# Agent Network Dev — Code Development Guide

## How Agent Network Works Now

Agent Network 已经不是 “`connect` 把单个 agent 连上平台” 的模型了，而是：
...
1. `cd agent-network && pnpm test`
2. `cd agent-network && pnpm build`
3. `cd .. && pnpm test`
4. `cd .. && pnpm run lint`
